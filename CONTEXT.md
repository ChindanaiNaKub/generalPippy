...
| **Review and critique** | Fresh-context inspection of a diff, touched files, acceptance criteria, and verification output before final verification. It produces findings for `pippy-build` to fix but is not a separate agent role. _Avoid_: Critic agent, cheap critic |
