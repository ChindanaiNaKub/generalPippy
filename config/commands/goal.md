...
### Verifier Template Rules

During acceptance-criteria shaping, choose the closest Verifier template and name it in the `Plan` evidence trail. Templates are evidence checklists, not separate modes, commands, or report fields:
...
If Pippy cannot identify a suitable template, ask one clarifying question or report a Goal readiness issue instead of inventing weaker evidence.
