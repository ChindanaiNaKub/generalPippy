...
- Force all bash commands through `rtk` when it is installed (e.g., `rtk ls`, `rtk git status --short`, `rtk git log`, `rtk git diff`, `rtk gh pr view`, `rtk make all`). Once `rtk` is available, treat the task as RTK-locked with no exploration grace period. Use `rtk run` or `rtk proxy` for commands without a specialized wrapper. Fall back to raw shell only when `rtk` is missing or cannot run that exact command; raw `git` after rtk is available is a routing failure.
