...
| Verification gates | `config/commands/goal.md`, `config/agents/pippy.md`, `config/skills/pippy/SKILL.md` | Acceptance criteria, Verifier templates, verification rigor, REVIEW, final verification, the review checklist, Program design check, and the Assumption audit |
...
| Model profiles | `config/model-profiles/budget.json`, `config/model-profiles/thorough.json`, `docs/agents/model-profiles.md`, `install.sh`, `docs/adr/0005-model-profiles-and-read-only-advisor-agents.md`, `docs/adr/0013-remove-advisor-adapters.md` | Coordination/planning/implementation/system model defaults |
...
| Update checks | `manifest.json`, `config/generalpippy/update-check.mjs`, `config/plugins/generalpippy-update-check.js`, `config/commands/pippy-update.md`, `install.sh`, `docs/adr/0015-pippy-update-check-and-release-manifest.md` | Release manifest metadata, installed version metadata, startup update notices, manual update checks, compatibility warnings, and consent-based installer launch |
...
| Goal-run evals | `docs/agents/goal-run-evals.md`, `scripts/goal-run-smoke-evals.sh`, `docs/agents/manual-smoke-tests.md`, `docs/agents/subagent-routing-smoke-test.md`, `docs/agents/caveman-mode-smoke-test.md` | Human-run checks for trajectory, routing, verification, retry behavior, improvement-signal quality, and installed behavior |
...
Do not add runtime hooks, schedulers, telemetry stores, or automatic self-modification as harness changes unless a separate decision record accepts that platform-level commitment. Accepted exceptions are `cc-safety-net`, a reviewed guardrail plugin added per [ADR-0010](../adr/0010-default-cc-safety-net-guardrail-plugin.md), and the local startup Pippy update check accepted in [ADR-0015](../adr/0015-pippy-update-check-and-release-manifest.md).
