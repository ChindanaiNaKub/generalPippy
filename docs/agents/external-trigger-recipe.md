...
```
/goal "check that README.md references the current version (v3.0.0), that README.md, CONTEXT.md, and all docs/agents/*.md files contain no stale v1.0 references, and that every file linked from README.md exists; report any drift as Blocked with the specific stale references"
```

**Observable acceptance criteria:**
- `bash tests/validate.sh` passes (exit 0)
- `bash scripts/doctor.sh` passes (exit 0)
- `grep -n 'v3.0.0' README.md` returns at least one match
