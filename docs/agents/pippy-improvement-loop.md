...
2. **Review the Improvement Signal.** The Improvement Signal field identifies Pippy-owned friction — problems in Pippy's own prompts, routing logic, acceptance-criteria shaping, context assembly, Program design handling, Verification gates, Verifier mismatch, Program design checks, or verification habits. It is always present and limited to Pippy-owned friction.
3. **Distinguish Pippy-owned friction from ordinary project failure.** Pippy-owned friction is things like: acceptance criteria were vague and had to be rewritten mid-run, a file was read multiple times because context was not compressed, the retry bundle omitted failure context so the first retry failed identically, a design-sensitive change skipped the Program design sketch, REVIEW accepted passing tests without Program design evidence, the Assumption audit missed an unsupported external-link/package claim, review routing produced findings but the final verification gate was skipped, the Verification gates trail reported all gates as pass while the Acceptance Criteria table or Outcome was partial, Pippy used raw `git`, `gh`, `make`, test commands, optional-tool probes, or baseline dirty-workspace checks after detecting `rtk` and still reported `Improvement Signal: None`, or Pippy omitted a raw command from the Plan and then claimed RTK Force was used throughout. Ordinary project failure is things like: the user's objective was genuinely blocked by a missing dependency, the codebase had pre-existing design debt unrelated to Pippy, the user explicitly chose a design trade-off, or the task required human judgment that no agent could resolve.
...
When an accepted Improvement Signal is useful for future `/goal` runs but is not yet stable enough to become a prompt, command, skill, ADR, or validation rule, a maintainer may promote it into cross-run memory. Weak or mismatched Verification gates are memory-worthy only after human review of a real failed run or accepted eval result. For example, a maintainer might record that documentation goals asking for exact wording or behavior need diff evidence for the exact request and a check that no unrelated files changed. See [cross-run-memory.md](cross-run-memory.md).
